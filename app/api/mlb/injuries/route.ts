...
const ESPN_URL = 'https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/injuries';
...
export async function GET() {
  try {
    ...
    serviceClient
      ...
      .then(({ error }) => {
        if (error) console.error('[API/MLB/Injuries] Cache write error:', error);
      });
    ...
    console.error('Failed to fetch MLB injuries:', error);
    return NextResponse.json([] as InjuryEntry[]);
  }
}
