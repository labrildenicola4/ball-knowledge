...
import { getNHLTeamRankings } from '@/lib/api-espn-nhl';
...
export async function GET() {
  try {
    ...
    const { data: cached, error: cacheError } = await supabase
      ...
      .eq('league_code', 'NHL_TEAM_RANKINGS')
    ...
    const rankings = await getNHLTeamRankings();
    ...
    serviceClient
      ...
      .then(({ error }) => {
        if (error) console.error('[API/NHL/TeamRankings] Cache write error:', error);
      });
    ...
    console.error('[API/NHL/TeamRankings] Error:', error);
    return NextResponse.json(
      { error: 'Failed to fetch NHL team rankings' },
      { status: 500 }
    );
  }
}
