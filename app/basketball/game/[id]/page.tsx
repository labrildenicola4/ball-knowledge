...
export default function BasketballGamePage() {
  ...
  const { data, error, isLoading, isValidating, mutate } = useSWR<{
    ...
    gameId ? `/api/basketball/games/${gameId}` : null,
    fetcher,
    {
      ...
      <section className={`px-4 py-8 ${darkMode ? 'glass-section' : ''}`} style={darkMode ? undefined : { backgroundColor: theme.bgSecondary }}>
        <div className="flex items-start justify-between">
          ...
          <Link href={`/basketball/team/${game.awayTeam.id}`} className="flex-1 text-center logo-press">
            ...
            {game.awayTeam.rank && (
              <span className="block text-[11px] font-bold mb-1" style={{ color: theme.accent }}>
                #{game.awayTeam.rank}
              </span>
            )}
                ...
                {isLive ? (
                  <div className="mt-3">
                    ...
                    >
                      ...
                      {game.period > 2 ? `OT${game.period - 2}` : game.period === 2 ? '2nd Half' : '1st Half'} {game.clock}
                    </span>
                  </div>
                ) : (
                  <span
